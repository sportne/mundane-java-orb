...
|---|---|---|---|---|---|
| Time Service | G8-100-TIME-SERVICE-TASK-GROUP, G8-110-TIME-SERVICE-IIOP-NAMING-EXPOSURE, G8-120-TIME-SERVICE-INTEROP-METADATA | REQ-SVC-060 | Approved peer manifests now declare `time-service` dry-run metadata for peer clients querying our Time Service and our clients querying peer Time Service endpoints. Live execution remains blocked until the G8-130 approval record and G8-140 closure. | G8-100 smoke covers value creation, local query, interval creation, and bounded diagnostics. G8-110 smoke also covers loopback IIOP and Naming-resolved Time Service calls. | Clock-source trust, overflow, precision loss, stale clock diagnostics, malformed request bodies, unknown object keys, unknown operations, and missing live peer prerequisites are bounded for the local/IIOP and metadata slices. |
