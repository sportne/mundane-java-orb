...
| Event Service | G8-200-EVENT-SERVICE-TASK-GROUP, G8-210-EVENT-SERVICE-CHANNEL-MODEL | REQ-SVC-020 | Future scenarios should cover push and pull channel interoperability after missing-prerequisite reports exist. | G8-210 unit coverage verifies channel creation, admin/proxy ownership, and bounded lifecycle diagnostics; Native Image smoke remains planned for G8-250. | Channel lifecycle, configured limits, destroyed-state diagnostics, and failure disclosure are bounded for the local model. Delivery backpressure remains planned for G8-230. |
