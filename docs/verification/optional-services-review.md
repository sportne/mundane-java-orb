...
|---|---|---|---|---|---|
| Time Service | G8-100-TIME-SERVICE-TASK-GROUP, G8-110-TIME-SERVICE-IIOP-NAMING-EXPOSURE, G8-120-TIME-SERVICE-INTEROP-METADATA, G8-130-TIME-SERVICE-LIVE-PEER-GATE | REQ-SVC-060 | Approved peer manifests declare `time-service` dry-run metadata for peer clients querying our Time Service and our clients querying peer Time Service endpoints. G8-130 records maintainer approval for live execution; G8-140 must still run the approved matrix and summarize evidence before compatibility is claimed. | G8-100 smoke covers value creation, local query, interval creation, and bounded diagnostics. G8-110 smoke also covers loopback IIOP and Naming-resolved Time Service calls. | Clock-source trust, overflow, precision loss, stale clock diagnostics, malformed request bodies, unknown object keys, unknown operations, and missing live peer prerequisites are bounded for the local/IIOP and metadata slices. |
