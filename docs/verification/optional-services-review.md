...
|---|---|---|---|---|---|
| Time Service | G8-100-TIME-SERVICE-TASK-GROUP | REQ-SVC-060 | Future scenarios should cover peer clients querying our Time Service and our clients querying peer Time Service endpoints after metadata exists. | G8-100 smoke covers value creation, local query, interval creation, and bounded diagnostics. | Clock-source trust, overflow, precision loss, and stale clock diagnostics are bounded for the local slice; IIOP/Naming exposure remains staged. |
