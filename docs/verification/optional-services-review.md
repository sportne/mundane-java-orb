...
|---|---|---|---|---|---|
| Time Service | G8-100-TIME-SERVICE-TASK-GROUP, G8-110-TIME-SERVICE-IIOP-NAMING-EXPOSURE | REQ-SVC-060 | Future scenarios should cover peer clients querying our Time Service and our clients querying peer Time Service endpoints after metadata exists. G8-110 has local loopback IIOP/Naming evidence only. | G8-100 smoke covers value creation, local query, interval creation, and bounded diagnostics. G8-110 smoke also covers loopback IIOP and Naming-resolved Time Service calls. | Clock-source trust, overflow, precision loss, stale clock diagnostics, malformed request bodies, unknown object keys, and unknown operations are bounded for the local/IIOP slice. |
