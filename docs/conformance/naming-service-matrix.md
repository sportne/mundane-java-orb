...
| stringified names | NAM-13-STRINGIFIED | REQ-NAM-001 | partial | `NamingNameTest`, `OmgCompatibilityApiTest` | G6-810 adds local slash/dot/backslash stringified-name parsing and formatting. G10-030 adds source-compatible `NamingContextExt` stringified-name method shapes. Runtime network behavior remains deferred. |
...
| list | NAM-13-CONTEXT, listing a naming context; NAM-13-ITERATOR | REQ-NAM-001 | partial | `NamingContextTest`, `OmgCompatibilityApiTest` | G6-810 adds local deterministic listing and snapshot iterator behavior. G10-030 adds source-compatible Binding, BindingListHolder, BindingIterator, and BindingIteratorHolder surfaces. |
