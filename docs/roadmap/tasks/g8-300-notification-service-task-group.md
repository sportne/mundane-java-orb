...
Expected behavior: Task type: implementation. Promoted by maintainer directive for the task-splitting slice. Split Notification Service into staged implementation tasks covering Event Service compatibility boundary, structured events, bounded filters, QoS/admin policy, local delivery, optional IIOP/Naming exposure, Native Image smoke, structured interop metadata, and conformance closure.
