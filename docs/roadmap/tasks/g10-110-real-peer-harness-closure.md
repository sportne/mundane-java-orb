...
Expected behavior: Task type: implementation. Replaced scaffold-only peer container contracts with black-box launchers for JacORB, Eclipse GlassFish CORBA ORB, JBoss OpenJDK ORB, and ACE/TAO; added opt-in approved cache preparation, digest-pinned base-image handling, deterministic health checks, IOR exchange paths, scenario orchestration, report aggregation, and clean-room failure classification.
...
Acceptance criteria: Harness can execute real peer scenarios only when approved cache, digest-pinned base images, Docker/Podman, and prepared peer images are present; no peer source/binaries or live outputs are committed.
