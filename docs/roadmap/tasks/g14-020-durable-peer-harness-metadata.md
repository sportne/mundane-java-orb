...
Expected behavior: Task type: implementation. G14-010 accepted the local durable evidence and promoted this task to `ready-for-implementation`; add dry-run harness metadata for durable IOR and persistent Naming peer scenarios without approving live execution. Scope every claim to peer preservation of opaque object-key bytes and ordinary IOR/Naming protocol behavior; peers must not parse or understand `MJOK` object keys or `MJNS` store files.
...
Completion evidence: Completed on 2026-05-31 by adding dry-run manifest metadata for `g13-durable-ior-peer-client-restart` and `g13-durable-naming-peer-client-restart` across JacORB, Eclipse GlassFish CORBA ORB, JBoss OpenJDK ORB, and ACE/TAO. Interop testkit coverage verifies scenario declarations, one-sided dry-run direction enumeration, and ignored raw-evidence paths without live execution.
