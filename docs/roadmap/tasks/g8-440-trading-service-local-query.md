...
Completion evidence: Added bounded local type-scoped query methods on `LocalTradingOfferRepository` plus `TradingQueryOptions`. Queries validate the service type, parse the bounded `TradingConstraint`, pre-check configured candidate cost, evaluate only local in-memory offers of the requested type, return deterministic offer-ID ordering, and reject result-count or cost limit violations with `TRAD-0021`. Unit tests cover exact and comparison matches, empty results, deterministic ordering, max-result rejection, query cost rejection before constraint evaluation, unknown type, malformed constraint, unknown property, type mismatch, invalid query limits, and type isolation. G8-450 is promoted as the only ready Trading Service implementation task.
