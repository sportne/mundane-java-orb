...
Completion evidence: ADR-0014 accepts durable ORB identity, persistent POA
identity, restart-safe object keys, persistent IORs, and Naming persistence as
the project's durable identity design direction. Architecture and conformance
documents now define the identity model, restart boundary, CDR/GIOP/IIOP
encoding constraints, Naming persistence scope, Native Image posture, and
security constraints. Follow-on roadmap tasks split implementation into an ORB
identity foundation, persistent POA object keys, persistent IOR round-trips, and
Naming persistence, with dependent tasks blocked until their prerequisites land.
