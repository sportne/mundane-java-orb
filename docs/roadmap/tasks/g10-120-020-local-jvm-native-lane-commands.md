...
Completion evidence: On 2026-05-25, focused `corba-interop-testkit` and `corba-native-image` tests passed, Native Image client/server binaries were rebuilt from SDKMAN GraalVM, and `run-direction-matrix --require-live basic-idl ace-tao` passed with the approved artifact cache, approved native base image, an explicit Docker network, and local JVM/native lane commands.
