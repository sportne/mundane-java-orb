...
Allowed files: interop/**, modules/corba-interop-testkit/src/**, modules/corba-iiop/src/**, modules/corba-rmi-iiop/src/**, modules/corba-native-image/src/nativeSmoke/java/**, modules/corba-native-image/src/test/**, docs/verification/interop-matrix.md, docs/verification/reference-behavior-capture.md, docs/verification/pre-1-0-interoperability-plan.md, docs/roadmap/roadmap-index.md, docs/roadmap/tasks/g10-120-*.md, README.md
...
Completion evidence: Completed on 2026-05-26. The task contract was amended before production fixes to allow the narrow IIOP, RMI-IIOP, and native-smoke compatibility surfaces needed by the live Java peer lanes. The fix adds code-set components to local IORs, code-set service contexts to RMI-IIOP requests/replies, flexible peer `wstring` decoding while retaining BOM-prefixed UTF-16 writes, corrected Java peer empty user-exception Any construction, and peer-aware local client handling for GlassFish/JBoss `clear` behavior. Focused tests passed for `:modules:corba-rmi-iiop:test`, `:modules:corba-native-image:test`, and `:modules:corba-interop-testkit:test`; `:modules:corba-native-image:nativeImageBinariesSmoke` rebuilt fresh native lane binaries. Live `rmi-iiop` direction matrices passed for JacORB, GlassFish CORBA ORB, and JBoss OpenJDK ORB with both JVM and Native Image local lanes, leaving zero `our-bug` classifications.
