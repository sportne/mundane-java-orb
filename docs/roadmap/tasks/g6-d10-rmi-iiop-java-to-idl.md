...
Acceptance criteria: ADR-0013 has status accepted; `docs/architecture/rmi-iiop-java-to-idl.md` defines scope, module boundaries, Native Image restrictions, security posture, interop impact, and future verification expectations; conformance matrices showed RMI-IIOP and Java-to-IDL as designed but not implemented at design-gate completion; G7 roadmap tasks cover the complete topic with G7-010 as the first implementation task; `modules/corba-rmi-iiop` remained scaffold-only for G6-D10; no runtime implementation was added by G6-D10.
