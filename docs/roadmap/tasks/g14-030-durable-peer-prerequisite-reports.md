...
Expected behavior: Task type: human-gate-blocked. This task is held by the G14-010 evidence-acceptance gate and the G14-020 metadata prerequisite, not by a separate human approval. After maintainers accept G13 local durable evidence and G14-020 metadata is ready, promote this task to `ready-for-implementation` and add structured report validation for durable peer scenarios when required peer images, artifact cache entries, native binaries, Docker, or live approval are missing.
