...
Expected behavior: Task type: implementation. Expanded RMI-IIOP beyond the earlier primitive/wstring/void/empty-exception slice to peer-facing Java-to-IDL compatibility: explicit declared-value metadata, deterministic remote object-reference payloads, repository IDs, user exceptions with payloads, inheritance metadata, and generated binding surfaces.
...
Acceptance criteria: RMI-IIOP peer scenarios are represented in local generated binding, CDR, and wire behavior without unapproved serialization, scanning, or reflection metadata. Live peer pass/fail execution remains deferred to G10-110 and G10-120.
