...
Expected behavior: Task type: implementation. Blocked until maintainers promote a Security Service / CSIv2 slice. Implement in staged steps: credential/trust model, policy model, CSIv2 metadata, local policy evaluation, audit/failure disclosure, IIOP integration, Native Image smoke, structured interop metadata, and conformance closure.
