...
Completion evidence: Added `LocalTradingTypeRepository` with bounded type registration, update, deletion, lookup, deterministic list snapshots, immutable `TradingServiceType` and `TradingPropertyDefinition` models, stable `TRAD-*` diagnostics, and caller-configured repository limits. Unit tests cover duplicate, missing, malformed, limit, property-kind, duplicate-property, and immutability paths. G8-420 is promoted as the only ready Trading Service implementation task.
