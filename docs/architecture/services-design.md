...
| Trading Service | REQ-SVC-010 | TRADE-10 | `modules/corba-trading-service` owns service type repositories, offer repositories, constraint parsing/evaluation, local query, and future import/export behavior. | G8-410 implements the bounded local service type repository with primitive property definitions and stable diagnostics. G8-420 implements bounded in-memory offer CRUD over registered service types with primitive property values, immutable snapshots, list-by-type ordering, and deterministic compatibility/limit diagnostics. G8-430 implements the closed-world bounded constraint parser/evaluator over primitive property maps. G8-440 implements bounded local type-scoped offer query with deterministic offer-ID ordering, configured result/cost limits, and clear constraint diagnostics. G8-450 is ready to add import/export boundary metadata before IIOP/Naming, Native Image, interop metadata, and closure tasks. |
...
evaluation, and local query, and import/export boundary metadata is the only
ready Trading slice; Transaction and Security/CSIv2 remain blocked behind their
task groups.
