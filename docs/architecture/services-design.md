...
| Security Service / CSIv2 | REQ-SVC-050 | SEC-18, CORBA-IOP-SECURITY | `modules/corba-security-service` owns credentials, trust, policy, CSIv2 metadata, local policy evaluation, audit/failure disclosure, descriptor-backed loopback IIOP security contexts, Native Image smoke coverage, and interop metadata for the implemented subset. | G8-690 closes the implemented Security Service conformance record across bounded credential/trust behavior, policy validation, CSIv2 metadata encode/decode, local allow/challenge/deny policy evaluation, redacted audit/failure disclosure, descriptor-backed loopback IIOP service-context and tagged-component handling, Native Image smoke coverage, source-level closed-world audits, and metadata-only `security-service` dry-run reporting. Live secure peer execution, pass/fail peer compatibility claims, enterprise identity management, automatic TLS policy changes, global JVM security-manager integration, generated OMG APIs, durable identity integration, Java serialization metadata, reflection metadata, dynamic proxies, and runtime bytecode generation remain unapproved. |
...
loopback IIOP boundary, G8-670 Native Image smoke evidence, and G8-680
metadata-only interop reporting. G8-690 closes the Security/CSIv2
local/IIOP/Native Image/dry-run conformance record without any live secure peer
claim.
