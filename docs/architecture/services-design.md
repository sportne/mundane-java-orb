...
| Security Service / CSIv2 | REQ-SVC-050 | SEC-18, CORBA-IOP-SECURITY | `modules/corba-security-service` owns credentials, trust, policy, CSIv2 metadata, local policy evaluation, audit/failure disclosure, descriptor-backed loopback IIOP security contexts, Native Image smoke coverage, and interop metadata for the implemented subset. | G8-610 implements the bounded local credential/trust model: principal, credential, and trust-anchor IDs, credential kinds, lifetime metadata, immutable snapshots, trust-evaluation input, stable diagnostics, configured limits, and deterministic trusted/untrusted decisions. G8-620 implements bounded policy validation for authentication, trust, transport protection, identity assertion, delegation, and audit settings with deterministic malformed, duplicate, conflicting, unsupported-delegation, and limit diagnostics. G8-630 implements bounded CSIv2 metadata primitives and project-owned deterministic encode/decode for the supported local mechanism identity, transport-protection flags, identity-token policy, and target/client authentication metadata. G8-640 implements deterministic local policy evaluation with allow, challenge, and deny decisions for missing, expired, untrusted, unsupported-delegation, insufficient-transport, malformed-metadata, and conflict diagnostics. G8-650 implements bounded redacted audit events and stable failure disclosures without raw exception-message or credential-material exposure. G8-660 implements descriptor-backed loopback IIOP service-context and tagged-component handling with local policy evaluation, malformed-context diagnostics, redacted audit events, and clean shutdown. G8-670 adds Native Image smoke coverage for credential rejection, trust validation, policy rejection, CSIv2 metadata encode/decode, local policy evaluation, audit redaction, IIOP boundary behavior, clean shutdown, and source-level closed-world audits. G8-680 adds metadata-only `security-service` interop discoverability: the SEC-18/CSIv2 IDL fixture, approved peer manifest declarations, JVM/native dry-run direction enumeration, and structured missing-prerequisite reports without live peer execution. Conformance closure remains staged behind G8-690. |
...
implemented subset. Security/CSIv2 now has the G8-610 credential/trust model,
G8-620 policy model, G8-630 CSIv2 metadata model, G8-640 local policy
evaluation model, G8-650 redacted audit/failure disclosure model, G8-660
loopback IIOP boundary, and G8-670 Native Image smoke evidence, with only the
G8-680 metadata-only interop slice complete. G8-690 conformance closure is the
only ready Security/CSIv2 successor.
