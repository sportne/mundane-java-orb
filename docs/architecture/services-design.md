...
| Transaction Service / OTS | REQ-SVC-040 | TRANS-14 | `modules/corba-transaction-service` owns transaction coordinator, resource, propagation, timeout, recovery-boundary, and request-context design decisions for the implemented local subset. | G8-550 records the recovery boundary with disabled durable recovery policy, stable diagnostics for unapproved durable logs/replay, and deterministic terminal-state behavior without durable replay. G8-560 is the only ready Transaction Service implementation slice. |
...
IIOP request-context boundary is the next ready slice. Security/CSIv2 remains
blocked behind its task group.
