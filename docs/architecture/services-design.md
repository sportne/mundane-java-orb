...
| Trading Service | REQ-SVC-010 | TRADE-10 | `modules/corba-trading-service` owns service type repositories, offer repositories, constraint parsing/evaluation, local query, and future import/export behavior. | G8-410 implements the bounded local service type repository with primitive property definitions and stable diagnostics. G8-420 implements bounded in-memory offer CRUD over registered service types with primitive property values, immutable snapshots, list-by-type ordering, and deterministic compatibility/limit diagnostics. G8-430 is ready to add the bounded constraint model before query, import/export, IIOP/Naming, Native Image, interop metadata, and closure tasks. |
...
Service has started with bounded type and offer repositories, and the bounded
constraint model is the only ready Trading slice; Transaction and Security/CSIv2
