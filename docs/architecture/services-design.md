...
| Transaction Service / OTS | REQ-SVC-040 | TRANS-14 | `modules/corba-transaction-service` owns transaction coordinator, resource, propagation, timeout, recovery-boundary, and request-context design decisions for the implemented local subset. | G8-510 implements the first local subset: bounded transaction/resource IDs, opaque local handles, immutable coordinator/resource snapshots, enlist/delist behavior, explicit forget/removal, caller-configured limits, and deterministic duplicate, missing, stale, malformed, and limit diagnostics. G8-520 is the only ready Transaction Service implementation slice. |
...
bounded local coordinator/resource model; timeout policy is the next ready
