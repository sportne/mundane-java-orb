...
| Trading Service | REQ-SVC-010 | TRADE-10 | `modules/corba-trading-service` owns service type repositories, offer repositories, constraint parsing/evaluation, local query, import/export boundary metadata, and loopback IIOP/Naming exposure. | G8-410 implements the bounded local service type repository with primitive property definitions and stable diagnostics. G8-420 implements bounded in-memory offer CRUD over registered service types with primitive property values, immutable snapshots, list-by-type ordering, and deterministic compatibility/limit diagnostics. G8-430 implements the closed-world bounded constraint parser/evaluator over primitive property maps. G8-440 implements bounded local type-scoped offer query with deterministic offer-ID ordering, configured result/cost limits, and clear constraint diagnostics. G8-450 implements bounded import/export link metadata with disabled-federation diagnostics while keeping query local-only. G8-460 implements descriptor-backed loopback IIOP/Naming for the supported local Trader facade with deterministic malformed request, stale object, unknown operation, disabled import, and shutdown diagnostics. G8-470 is ready to add Native Image smoke before interop metadata and closure tasks. |
