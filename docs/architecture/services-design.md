...
| Transaction Service / OTS | REQ-SVC-040 | TRANS-14 | `modules/corba-transaction-service` owns transaction coordinator, resource, propagation, timeout, recovery-boundary, and request-context design decisions for the implemented local subset. | G8-540 extends the local subset with immutable local propagation contexts, deterministic bounded text encode/decode, and unknown/stale/expired/malformed context diagnostics without IIOP request-context integration. G8-550 is the only ready Transaction Service implementation slice. |
...
transitions, and propagation metadata; the recovery boundary is the next ready
