...
| Notification Service | REQ-SVC-030 | NOT-11 | `modules/corba-notification-service` owns notification channels, structured events, bounded filtering, QoS/admin models, local delivery, optional IIOP/Naming exposure, Native Image smoke, interop metadata, and Event Service compatibility boundaries. | G8-300 splits implementation into narrow slices. G8-310 implements the local channel lifecycle and Event Service compatibility boundary with bounded options, stable diagnostics, supplier/consumer admin handles, structured proxy lifecycle handles, and proxy-role mapping back to Event Service roles. G8-320 implements the immutable structured-event model with fixed identity, primitive named filter properties, bounded variable header/body fields, and deterministic validation. G8-330 implements an isolated bounded filter parser/evaluator over identity fields and primitive filter properties. G8-340 implements bounded QoS/admin policy validation for channel, admin, proxy, queue, filter, durable, and transaction policy keys. G8-350 implements in-JVM structured push/pull delivery with bounded filters, queues, failed-consumer removal, stale-proxy diagnostics, and destroyed-channel diagnostics. G8-360 implements descriptor-backed loopback IIOP/Naming exposure with structured push/pull operations, local filter/QoS rejection diagnostics, malformed request diagnostics, Naming-resolved NotificationChannel IORs, and clean shutdown. G8-370 implements representative Native Image smoke coverage for channel creation, structured-event validation, filter validation, QoS rejection, local delivery, loopback IIOP/Naming exposure, and clean shutdown. G8-380 adds metadata-only `notification-service` peer discovery, the NOT-11 IDL fixture, JVM/native dry-run direction enumeration, and structured missing-prerequisite reports. Closure remains staged follow-on work; live peer execution remains unapproved. |
