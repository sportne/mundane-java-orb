...
| Transaction Service / OTS | REQ-SVC-040 | TRANS-14 | `modules/corba-transaction-service` owns transaction coordinator, resource, propagation, timeout, recovery-boundary, and request-context design decisions for the implemented local subset. | G8-570 adds Native Image smoke coverage and source-level closed-world audits for coordinator/resource creation, timeout rejection, local commit/rollback paths, propagation metadata validation, request-context boundary behavior, disabled recovery diagnostics, and clean shutdown. G8-580 is the only ready Transaction Service implementation slice. |
...
for the implemented subset. Transaction Service / OTS has started with the
bounded local coordinator/resource model, explicit timeout policy, local state
transitions, propagation metadata, and disabled durable-recovery boundary; the
IIOP request-context boundary now carries the supported local propagation subset
without peer distributed two-phase commit, and Native Image smoke covers the
local/IIOP-adjacent boundary without metadata files. Metadata-only interop is
the next ready slice. Security/CSIv2 remains blocked behind its task group.
