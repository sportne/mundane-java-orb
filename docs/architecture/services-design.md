...
| Transaction Service / OTS | REQ-SVC-040 | TRANS-14 | `modules/corba-transaction-service` owns transaction coordinator, resource, propagation, timeout, recovery-boundary, and request-context design decisions for the implemented local subset. | G8-560 adds the descriptor-backed loopback IIOP request-context boundary with stable service-context IDs and bytes, malformed/duplicate context diagnostics, stale/unknown/expired context rejection through the local coordinator, and clean shutdown. G8-570 is the only ready Transaction Service implementation slice. |
...
without peer distributed two-phase commit. Native Image smoke is the next ready
slice. Security/CSIv2 remains blocked behind its task group.
